...
***************************************************************************** */
  ...
  var BaseTransition = (function (_super) {
    function BaseTransition() {
      var _this = (null !== _super && _super.apply(this, arguments)) || this;
      return (_this.leavingVNodesCache = {}), (_this.el = null), _this;
    }
    return (
      tslib_es6_extends(BaseTransition, _super),
      (BaseTransition.template = function () {
        var props = this.props,
          children = props.children;
        if (index_esm_isNullOrUndefined(children)) return null;
        var vNode = index_esm_isArray(children) ? children[0] : children;
        return (
          setTransitionHooks(vNode, resolveTransitionHooks(vNode, props, this)),
          vNode
        );
      }),
      (BaseTransition.prototype.defaults = function () {
        return { show: !0 };
      }),
      (BaseTransition.prototype.mounted = function () {
        var lastInput = this.$lastInput;
        if (!(1 & lastInput.type)) {
          var el = (this.el = index_esm_findDomFromVNode(lastInput, !0)),
            display = el.style.display,
            originDisplay = "none" === display ? "" : display;
          this.props.show || setDisplay(el, "none"),
            this.on("$receive:show", function (show) {
              show
                ? (lastInput.transition.beforeEnter(el),
                  setDisplay(el, originDisplay),
                  lastInput.transition.enter(el))
                : lastInput.transition.leave(el, function () {
                    setDisplay(el, "none");
                  });
            });
        }
      }),
      (BaseTransition.prototype.unmounted = function () {
        var el = this.el;
        el && (el.$TD = void 0);
      }),
      BaseTransition
      ...
      return (
        ...
        (Component.prototype.set = function (key, value, options) {
          ...
          if (
            ...
            for (var propName in key) set(this.props, propName, key[propName]);
        }),
          ...
          (Event.prototype.off = function (name, callback) {
            if (index_esm_isUndefined(name)) this.$events = {};
              ...
              if (!index_esm_isUndefined(callbacks))
                if (index_esm_isUndefined(callback))
  ...
  function resolveTransitionHooks(vNode, props, component) {
    var appear = props.appear,
      ...
      key = vNode.key,
      leavingVNodesCache = component.leavingVNodesCache;
    return {
      beforeEnter: function (el) {
        ...
        if (leavingVNode && vNode.tag === leavingVNode.tag) {
          var leaveCb = index_esm_findDomFromVNode(leavingVNode, !0)._leaveCb;
          leaveCb && leaveCb();
        }
        hook && hook(el);
      },
      enter: function (el) {
        var hook = onEnter,
          afterHook = onAfterEnter,
          cancelHook = onEnterCancelled;
        if (!component.$mounted) {
          if (!appear) return;
        ...
        var called = !1,
          done = (el._enterCb = function (cancelled) {
            called ||
              ((called = !0),
              cancelled
        ...
        hook ? (hook(el, done), hook.length <= 1 && done()) : done();
      },
      leave: function (el, remove) {
        if ((el._enterCb && el._enterCb(!0), component.$unmounted))
          return remove();
        onBeforeLeave && onBeforeLeave(el);
        var called = !1,
          done = (el._leaveCb = function (cancelled) {
            called ||
              ((called = !0),
              remove(),
              cancelled
        ...
        (leavingVNodesCache[key] = vNode),
          onLeave ? (onLeave(el, done), onLeave.length <= 1 && done()) : done();
      },
    };
  }
  function setTransitionHooks(vNode, hooks) {
    if (8 & vNode.type) {
      ...
      !index_esm_isNullOrUndefined(component) && component.$inited
  ...
  var transitionProp = "transition",
    ...
    endEvents = [],
    nodeOps = (function () {
      var addClass, removeClass, raf, caf;
      if (hasDocumentAvailable) {
        ...
        if (testEl.classList)
          ...
          var hasClass_1 = function (dom, className) {
            return (
          ...
          (addClass = function (dom, className) {
            return (
              !hasClass_1(dom, className) && (dom.className += " " + className)
            );
          }),
            (removeClass = function (dom, className) {
              hasClass_1(dom, className) &&
        ...
        var EVENT_NAME_MAP = {
          ...
          style = testEl.style;
        for (var baseEventName in ("AnimationEvent" in window ||
    ...
    TransitionEvents_on = function (dom, eventListener) {
      0 !== endEvents.length
        ? endEvents.forEach(function (endEvent) {
            dom.addEventListener(endEvent, eventListener, !1);
          })
        : window.setTimeout(eventListener, 0);
    },
    TransitionEvents_off = function (dom, eventListener) {
      0 !== endEvents.length &&
        endEvents.forEach(function (endEvent) {
          dom.removeEventListener(endEvent, eventListener, !1);
        });
    };
  function nextFrame(fn) {
    ...
    return function () {
      caf(frame), caf(innerFrame);
    };
  }
  function whenTransitionEnds(dom, cb) {
    var _a = (function (dom) {
        var styles = window.getComputedStyle(dom),
          getStyleProperties = function (key) {
            return (styles[key] || "").split(", ");
          },
          transitionDelays = getStyleProperties(transitionProp + "Delay"),
          transitionDurations = getStyleProperties(transitionProp + "Duration"),
          transitionTimeout = getTimeout(transitionDelays, transitionDurations),
          animationDelays = getStyleProperties(animationProp + "Delay"),
          animationDurations = getStyleProperties(animationProp + "Duration"),
          animationTimeout = getTimeout(animationDelays, animationDurations),
          type = transitionTimeout > animationTimeout ? 0 : 1,
          timeout = Math.max(transitionTimeout, animationTimeout),
          propCount =
            0 === type ? transitionDurations.length : animationDurations.length;
      ...
      ended = 0,
      onEnd = function (e) {
        e.target === dom && ++ended >= propCount && end();
      },
      end = function () {
        TransitionEvents_off(dom, onEnd), clearTimeout(timer), cb();
      },
      timer = setTimeout(function () {
        ended < propCount && end();
      }, timeout + 1);
    TransitionEvents_on(dom, onEnd);
  }
  ...
  function resolveTransitionProps(props) {
    var _a = props.name,
      name = void 0 === _a ? "transition" : _a,
      _b = props.css,
      css = void 0 === _b || _b,
      _c = props.enterFromClass,
      enterFromClass = void 0 === _c ? name + "-enter-from" : _c,
      _d = props.enterActiveClass,
      enterActiveClass = void 0 === _d ? name + "-enter-active" : _d,
      _e = props.enterToClass,
      enterToClass = void 0 === _e ? name + "-enter-to" : _e,
      _f = props.appearFromClass,
      appearFromClass = void 0 === _f ? enterFromClass : _f,
      _g = props.appearActiveClass,
      appearActiveClass = void 0 === _g ? enterActiveClass : _g,
      _h = props.appearToClass,
      appearToClass = void 0 === _h ? enterToClass : _h,
      _j = props.leaveFromClass,
      leaveFromClass = void 0 === _j ? name + "-leave-from" : _j,
      _k = props.leaveActiveClass,
      leaveActiveClass = void 0 === _k ? name + "-leave-active" : _k,
      _l = props.leaveToClass,
      leaveToClass = void 0 === _l ? name + "-leave-to" : _l,
      baseProps = (function (s, e) {
        var t = {};
        for (var p in s)
    ...
    var onBeforeEnter = baseProps.onBeforeEnter,
      ...
      _m = baseProps.onBeforeAppear,
      onBeforeAppear = void 0 === _m ? onBeforeEnter : _m,
      _o = baseProps.onAppear,
      onAppear = void 0 === _o ? onEnter : _o,
      _p = baseProps.onAppearCancelled,
      onAppearCancelled = void 0 === _p ? onEnterCancelled : _p,
      cancelNextFrame = null,
      cancelFrame = function () {
        cancelNextFrame &&
          (cancelNextFrame(), forceReflow(), (cancelNextFrame = null));
      },
      finishEnter = function (el, isAppear, done) {
        isAppear
          ? (removeTransitionClass(
              el,
              cancelNextFrame ? appearFromClass : appearToClass
            ),
            removeTransitionClass(el, appearActiveClass))
          : (removeTransitionClass(
              el,
              cancelNextFrame ? enterFromClass : enterToClass
            ),
            removeTransitionClass(el, enterToClass),
            removeTransitionClass(el, enterActiveClass)),
          done && done();
      },
      finishLeave = function (el, done) {
        removeTransitionClass(
          el,
          cancelNextFrame ? leaveFromClass : leaveToClass
        ),
          removeTransitionClass(el, leaveActiveClass),
          done && done();
      },
      makeEnterHook = function (isAppear) {
        ...
        addTransitionClass(el, leaveFromClass),
          ...
          (cancelNextFrame = nextFrame(function () {
            removeTransitionClass(el, leaveFromClass),
              ...
              (index_esm_isUndefined(onLeave) || onLeave.length <= 1) &&
                whenTransitionEnds(el, resolve),
              (cancelNextFrame = null);
          }));
      },
  ...
  (compile = function () {}),
    ...
    $props || ($props = {}),
    (tag = Transition),
    (props = { children: index_esm_createElementVNode(2, "div", "test", 16) }),
    createComponentVNode(4, tag, props, key, ref);
})();
